...
const SoilChemInput = () => {
  ...
  return (
    <div className="soil-chem-input">
      {Object.entries(attributes).map(([label, { value, minValue, maxValue }]) => (
        <div key={label} className="range-input">
          ...
          <input
            ...
            value={value}
            onChange={(e) => handleInputChange(label, parseFloat(e.target.value))}
          />
          <span>{value}</span>
        </div>
      ))}
